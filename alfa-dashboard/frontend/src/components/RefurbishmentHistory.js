...
export default function RefurbishmentHistory(){
  ...
  const fetchList = async ()=>{
    try{
      setLoading(true);
      const res = await axios.get(`${API_BASE}/api/refurbishments`, { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } });
  ...
  const filtered = items.filter(i => {
    ...
    return car.includes(q) || (i.items || []).some(it => (it.description || '').toLowerCase().includes(q));
  });

  if(loading) return <div style={{ display:'flex' }}><Sidebar /><div style={{ padding:24 }}>Loading...</div></div>;
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12 }}>
          ...
          <div style={{ display:'flex', gap:8 }}>
            <div style={{ position:'relative' }}>
              ...
              <input placeholder='Search by car or item...' value={query} onChange={e=> setQuery(e.target.value)} style={{ padding:'8px 8px 8px 32px', borderRadius:8, border:'1px solid #e6edf3' }} />
            </div>
          </div>
        </div>

        <div style={{ background:'#fff', padding:12, borderRadius:8 }}>
          {filtered.length === 0 ? <div style={{ padding:24, color:'#6b7280' }}>No refurbishments found</div> : (
              ...
              <tbody>
                {filtered.map((r, idx) => (
                  <tr key={r._id} style={{ borderBottom:'1px solid #f3f4f6' }}>
                    ...
                    <td style={{ padding:10 }}><div style={{ fontWeight:700 }}>{r.car ? `${r.car.make} ${r.car.model}` : 'Unknown'}</div></td>
                    <td style={{ padding:10 }}>
                    ...
                    <td style={{ padding:10, verticalAlign:'top' }}>₹{(r.totalCost || 0).toLocaleString()}</td>
                    <td style={{ padding:10 }}>{new Date(r.createdAt).toLocaleString()}</td>
                    <td style={{ padding:10 }}>
                      <div style={{ display:'flex', gap:8 }}>
                        <button onClick={()=> setSelected(r)} style={{ background:'#f3f4f6', border:'1px solid #e6edf3', padding:'6px 8px', borderRadius:6 }}><Eye size={14} /></button>
                        { (user?.role === 'admin' || (r.user && r.user === user?._id)) && (
                          <button onClick={()=> deleteItem(r._id)} style={{ background:'#ef4444', color:'#fff', border:'none', padding:'6px 8px', borderRadius:6 }}><Trash size={14} /></button>
                        ) }
                      </div>
                    </td>
                  </tr>
        ...
        {selected && (
          <div style={{ marginTop:12, background:'#fff', padding:12, borderRadius:8 }}>
            ...
            <div style={{ marginTop:8 }}>
              ...
              <div style={{ marginTop:8, fontWeight:700 }}>Total: ₹{(selected.totalCost || 0).toLocaleString()}</div>
            </div>
          </div>
        )}
      </div>
