...
exports.createPayment = async (req, res) => {
  try {
    ...
    const payment = new AdvancePayment({ sellLetter, amount: Number(amount), paymentMethod, note, receivedBy: req.user.id });
    await payment.save();
...
exports.getPayments = async (req, res) => {
  try {
    ...
    // Admins see all, others see only payments they received or created
    if (req.user.role !== 'admin') {
      query.receivedBy = req.user.id;
    }

    const payments = await AdvancePayment.find(query).populate('sellLetter').populate('receivedBy', '-password').sort({ createdAt: -1 });
...
exports.getPayment = async (req, res) => {
  try {
    const payment = await AdvancePayment.findById(req.params.id).populate('sellLetter');
    if (!payment) return res.status(404).json({ success: false, message: 'Not found' });

    if (req.user.role !== 'admin' && payment.receivedBy.toString() !== req.user.id.toString()) {
